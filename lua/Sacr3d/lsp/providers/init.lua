...
for config_server, config_opt in pairs(config.lsp.servers) do
	...
	elseif
		not vim.tbl_contains(requested_servers, config_server)
	then
...
local lsp_installer_servers = require(
	'nvim-lsp-installer.servers'
)
for _, requested_server in pairs(requested_servers) do
	local ok, server = lsp_installer_servers.get_server(
		requested_server
	)
	if ok then
...
lsp_installer.on_server_ready(function(server)
	...
	if server.name == 'tsserver' then
		opts = vim.tbl_deep_extend(
			'force',
			opts,
			require('Sacr3d.lsp.providers.tsserver')
		)
	elseif server.name == 'sumneko_lua' then
		opts = vim.tbl_deep_extend(
			'force',
			opts,
			require('Sacr3d.lsp.providers.lua')
		)
	end
